...
#detects missing recipes and checks them with player interaction
    #fiber recipe
execute as @a if score @s openedBarrel > @s Zero run execute as @e[tag=Creation_Table, limit=1] at @s positioned ~ ~1 ~ store result score @s recipeMissing run data get block ~ ~ ~ Items[{Slot:1b}].Count 1
